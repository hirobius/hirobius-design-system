...
import {
  Type as TextAa, Palette, Zap as Lightning, Ruler, Box as Cube,
} from 'lucide-react';
...
export function Swatch({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: hds.semantic.space.subgrid.gap, alignItems: 'flex-start' }}>
      <div
        data-inspector-ignore="color-swatch"
        style={{ width: w, height: h, background: hex, flexShrink: 0, outline: showBorder ? `${hds.borderWidth.default} solid var(--semantic-color-border-default)` : undefined }}
      />
      {label    && <span className="font-normal text-secondary" style={{ ...hds.typeStyles.caption }}>{label}</span>}
      {sublabel && <span className="font-normal text-secondary" style={{ ...hds.typeStyles.caption }}>{sublabel}</span>}
    </div>
  );
}
...
export const TYPE_PRESETS: Array<{ name: keyof typeof hds.typeStyles; usage: string }> = [
  { name: 'display',   usage: 'Section panel titles (SectionHead). Max one per panel. Never body or ui.' },
  ...
  { name: 'ui',        usage: 'Descriptive interface labels, table headers, and human-readable metadata. 14px / 400 / normal tracking.' },
  { name: 'technical', usage: 'Token names, state identifiers, property names, and code-adjacent values. 12px / mono / high-signal.' },
  { name: 'body',      usage: 'Project descriptions, explanatory copy, and technical notes. Prefer body for long-form documentation prose. 16px / normal tracking.' },
  { name: 'caption',   usage: 'Secondary meta, timestamps, annotation pills, and short supporting notes. 12px / 400 / normal tracking.' },
...
export const LETTER_SPACING = Object.entries(hds.letterSpacing).map(([k, v]) => ({ key: k, val: v as string }));
...
export const TOKEN_REF_GROUPS: Array<{
...
}> = [
  {
    ...
    entries: [
      { path: 'primitive.typography.size.2xs', value: '10px', css: '--primitive-typography-size-2xs' },
      { path: 'primitive.typography.size.xs',  value: '12px', css: '--primitive-typography-size-xs'  },
      { path: 'primitive.typography.size.sm',   value: '14px', css: '--primitive-typography-size-sm'  },
      { path: 'primitive.typography.size.base', value: '16px', css: '--primitive-typography-size-base' },
      { path: 'primitive.typography.size.lg',   value: '20px', css: '--primitive-typography-size-lg'  },
      { path: 'primitive.typography.size.xl',   value: '24px', css: '--primitive-typography-size-xl'  },
    ],
  },
  {
    ...
    icon: <Icon icon={TextAa} size="small" color="currentColor" />,
    entries: [
      { path: 'primitive.typography.letterSpacing.tighter', value: '-0.8px', css: '--primitive-typography-letterSpacing-tighter' },
      { path: 'primitive.typography.letterSpacing.tight',   value: '-0.4px', css: '--primitive-typography-letterSpacing-tight'   },
      { path: 'primitive.typography.letterSpacing.normal',  value: '0px',    css: '--primitive-typography-letterSpacing-normal'  },
      { path: 'primitive.typography.letterSpacing.wide',    value: '0.4px',  css: '--primitive-typography-letterSpacing-wide'    },
      { path: 'primitive.typography.letterSpacing.wider',   value: '0.8px',  css: '--primitive-typography-letterSpacing-wider'   },
    ],
  },
  {
    group: 'Icon Size',
    ...
    entries: [
      { path: 'primitive.typography.size.base', value: '16', css: '--primitive-typography-size-base' },
  ...
  },
  {
    ...
    icon: <Icon icon={Lightning} size="small" color="currentColor" />,
    entries: [
      { path: 'primitive.easing.emphasized',      value: '[.4,0,.2,1]',      css: '--primitive-easing-emphasized'      },
      { path: 'primitive.easing.decelerate',      value: '[0,0,.2,1]',       css: '--primitive-easing-decelerate'      },
      { path: 'primitive.easing.accelerate',      value: '[.4,0,1,1]',       css: '--primitive-easing-accelerate'      },
      { path: 'primitive.easing.elastic',         value: 'spring(300,20,1)', css: '--primitive-easing-elastic'         },
    ],
  },
  {
    group: 'Color Brand',
    ...
    entries: [
      { path: 'semantic.accent.rest',   value: tokenValues.primitive.color.blue['500'], css: '--semantic-accent-rest'   },
      { path: 'semantic.accent.hover',  value: tokenValues.primitive.color.blue['600'], css: '--semantic-accent-hover'  },
      { path: 'primitive.color.neutral.white', value: tokenValues.primitive.color.neutral.white, css: '--primitive-color-neutral-white' },
    ],
  },
  {
    ...
    entries: [
      ...
      { path: 'semantic.space.subgrid.hairline', value: '2px',  css: '--semantic-space-subgrid-hairline' },
      { path: 'semantic.space.component.gap',   value: '8px',  css: '--semantic-space-component-gap'   },
      { path: 'semantic.space.component.padding', value: '12px', css: '--semantic-space-component-padding' },
      { path: 'semantic.space.layout.gutter',   value: '24px', css: '--semantic-space-layout-gutter'   },
...
export const SURFACE_TIERS = Object.entries(hds.color.surface) as Array<[string, { dark: string; light: string }]>;
export const FEEDBACK_COLORS = Object.entries(hds.color.feedback) as Array<[string, { dark: string; light: string }]>;
...
export const EASING_CURVES = [
  {
    ...
    path: bezierPath(0.40, 0, 0.20, 1),
    note: 'Balanced midpoint curve with symmetrical acceleration and deceleration.',
  },
  {
    label: 'decelerate',
    values: '[0, 0, 0.20, 1]',
    path: bezierPath(0, 0, 0.20, 1),
    note: 'High initial velocity that eases into a controlled stop.',
  },
];

export const DURATION_STEPS = [
  { name: 'instant', value: hds.motion.exit.duration, label: '0.10s', note: 'No delay for immediate state flips or removals.' },
  { name: 'short',   value: hds.motion.productive.duration, label: '0.15s', note: 'Hover reveals, tooltip fade, micro-interactions.' },
  { name: 'medium',  value: hds.motion.expressive.duration, label: '0.25s', note: 'Prominent surface changes and teaching moments.' },
  { name: 'long',    value: hds.motion.spatial.duration, label: '0.40s', note: 'Long-distance movement across the viewport.' },
];
...
export const LAYOUT_NOTES: Array<{ name: keyof typeof hds.layout; note: string }> = [
  { name: 'pageGutterH',   note: 'Page horizontal gutter — left + right padding on all full-width sections' },
  { name: 'mobileGutterH', note: 'Mobile horizontal gutter — narrower clamp range for small viewports'       },
  { name: 'sectionPad',    note: 'Section vertical padding — breathing room above/below section content'      },
  { name: 'sectionPadSm',  note: 'Compact section pad — used in dense panels and compact media headers'       },
...
export function DemoTag({ label, active, onClick }: { label: string; active: boolean; isDark: boolean; onClick?: () => void }) {
  return <Tag active={active} onClick={onClick ?? (() => {})}>{label}</Tag>;
}
...
export function TooltipDemo({ isDark: _isDark }: { isDark: boolean }) {
  ...
  return (
    ...
    >
      <span className="font-normal" style={{ ...hds.typeStyles.caption, color: 'var(--semantic-color-content-disabled)', userSelect: 'none' }}>
...
export function CascadePreview({ isDark: _isDark }: { isDark: boolean }) {
  ...
  return (
    <Surface
      ...
      style={{background:      'var(--semantic-color-surface-raised)',
        display:         'flex',
    ...
    >
      <span className="text-primary" data-inspector-ignore="motion-text" style={{ ...hds.typeStyles.display }}>
